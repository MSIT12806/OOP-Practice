...
1. 重構 EmpCore
	...
	我對單元測試做了改變，變成像是整合測試那樣，實際地傳送 request 到後端，走整個流程。這樣我可以不用拘泥於一個單元的行為是否完整，可以把該行為分散在整個請求的過程中，整合了整個 pipe line 的行為達成測試。

	接下來，我要把我的業務邏輯組件化，這樣我就可以確保我的 MVC 拓樸是乾淨的。


1. payway 何去何從？

	原本我的 payway 是用 Enum 的形式去處理，但是從 Enum 到多型的轉換，實在有點多此一舉，後來我決定用單純的 string作為資料型態，但是在附值的過程中一律使用 nameof ，算是一種強型別的保護。

	要思考的是，payway 到底算是 Basic Data Maintenance 下的資料，還是 Payroll 下的資料？



## 20204/11/7

好久不見，最近想要替這個系統加入權限驗證，因此跑去研究 .net Identity 這個套件。

為了讓 Identity 更好的適用 DDD 的架構，我給 Identity 加了一點包裝。

詳細的實作我就不公布出來了，今天我們直接來給這個系統加入不同的使用者。

1. 這個系統主要給 HR 使用，並且也可以給 員工使用，他們可以查詢他們自己的資料。

### 其他想要做的練習
1. 消除基本型別依賴
1. 引入 Entity 和 Value Object 的概念
1. 使用 多型 在程式碼當中，並且為多型的概念寫工具