...
1. 重構 EmpCore
	...
	然後，測試腳本也要重寫...因為現在變成要強制去確認前端要建立的員工是哪種類型的員工。

	一個壞消息，就是SaveChages 要回來了...為了讓單元測試順利通過，不能用Dispose 的時候自動 SaveChanges，因為這樣會讓測試腳本無法控制 SaveChanges 的時機，所以我決定把 SaveChanges 放回 EmpRepository 裡面。
### 其他想要做的練習
1. 基本型別依賴